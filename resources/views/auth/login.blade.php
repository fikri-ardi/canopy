<x-layouts.guest title="Masuk">
    <section class="panel px-5 py-6 sm:px-6">
        <div>
            ...
            <p class="page-subtitle">Masuk untuk membuka pengelolaan rencana milikmu.</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="email" name="email" type="email" value="{{ old('email') }}" autocomplete="email" autofocus placeholder="nama@email.com" class="input-field">
                @error('email')
            ...
            <div>
                ...
                <input id="password" name="password" type="password" autocomplete="current-password" placeholder="Masukkan password" class="input-field">
                @error('password')
                    <div class="mt-1 text-xs text-red-500">{{ $message }}</div>
                @enderror
            </div>
        ...
        <p class="mt-5 text-center text-sm text-gray-500 dark:text-slate-400">
            Belum punya akun?
            <a href="{{ route('register') }}" wire:navigate class="font-semibold text-green-600 hover:text-green-700 dark:text-green-400">Daftar</a>
        </p>
    </section>
</x-layouts.guest>

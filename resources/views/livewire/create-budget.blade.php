<div x-show="createBudget" x-cloak x-transition class="modal-backdrop">
    <div x-on:click.away="createBudget = false" class="modal-panel">
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="text-lg font-semibold text-gray-950 dark:text-slate-50">Buat Rencana</div>
                <p class="text-sm text-gray-500 dark:text-slate-400">Mulai rencana pemasukan baru.</p>
            </div>
        </div>
        ...
        <form class="mt-5 space-y-5" wire:submit="store">
            <div class="space-y-3">
                <div>
                    <label for="budget-name" class="mb-1 block text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Nama</label>
                    <input wire:model="name" x-on:blur="advanceBudgetName($event.target.value)" x-on:keydown.enter.prevent="advanceBudgetName($event.target.value)" type="text" name="name" id="budget-name" placeholder="Rencana bulanan" class="input-field" data-onboarding-target="budget-name">
                    @error('name')
                        <div class="mt-1 text-xs text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                <div>
                    <label for="budget-income" class="mb-1 block text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Pemasukan</label>

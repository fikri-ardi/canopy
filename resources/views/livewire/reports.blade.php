            ...
            <div class="page-header-actions">
                <div class="min-w-0 flex-1 sm:w-44 sm:flex-none">
                    <button x-ref="budgetTrigger" type="button" x-on:click.stop="budgetMenu.toggle($refs.budgetTrigger, $refs.budgetMenu)" class="btn-secondary w-full justify-between">
                        <span class="truncate">{{ $budgetId === 'all' ? 'Semua rencana' : $budgets->firstWhere('id', (int) $budgetId)?->name }}</span>
                    ...
                    <template x-teleport="body">
                        <div x-ref="budgetMenu" x-show="budgetMenu.open" x-cloak x-transition x-bind:style="budgetMenu.style" x-on:click.outside="budgetMenu.close()" x-on:resize.window="budgetMenu.close()" wire:key="reports-budget-menu" wire:ignore.self class="floating-select-menu">
                            <button type="button" x-on:click="budgetMenu.close()" wire:click="$set('budgetId', 'all')" class="w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 dark:text-slate-200 dark:hover:bg-slate-800">Semua rencana</button>
                ...
                <div class="min-w-0 flex-1 sm:w-40 sm:flex-none">
                    @php($rangeLabels = ['all' => 'Semua waktu', '30' => '30 hari terakhir', '90' => '90 hari terakhir', '365' => 'Setahun terakhir'])
                    <button x-ref="rangeTrigger" type="button" x-on:click.stop="rangeMenu.toggle($refs.rangeTrigger, $refs.rangeMenu)" class="btn-secondary w-full justify-between">
                        <span class="truncate">{{ $rangeLabels[$range] ?? 'Semua waktu' }}</span>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4 shrink-0">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
                    </button>
    ...
    <main class="space-y-6 px-4 py-5 sm:px-6 sm:py-6 lg:px-8">
        <section class="summary-grid">
            <div class="metric-card">
                <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Rencana Pemasukan</div>
                <div class="metric-value money-value">{{ $this->rupiah($totalIncome) }}</div>
            </div>
            <div class="metric-card">
                <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Pengeluaran</div>
        ...
        <section class="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)]">
            <div class="panel px-4 py-4">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <h2 class="mt-1 text-base font-bold text-gray-950 dark:text-slate-50">Progress rencana</h2>
                    </div>
                </div>
                ...
                <div class="mt-5 space-y-4">
                        ...
                        <div wire:key="reports-budget-progress-{{ $budget['id'] ?? str($budget['name'])->slug() }}">
                            <div class="mb-2 flex items-center justify-between gap-3 text-sm">
                                <div class="min-w-0">
                                    ...
                                    <div class="money-value text-xs text-gray-500 dark:text-slate-400">{{ $this->rupiah($budget['spent']) }} dari {{ $this->rupiah($budget['income']) }}</div>
                                </div>
                    ...
                    @empty
                        <div class="rounded-lg border border-dashed border-gray-200 px-4 py-8 text-center text-sm text-gray-500 dark:border-slate-700 dark:text-slate-400">Belum ada rencana.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="panel px-4 py-4">
                ...
                <div class="mt-4 divide-y divide-gray-100 dark:divide-slate-800">
                    ...
                    @empty
                        <div class="py-8 text-center text-sm text-gray-500 dark:text-slate-400">Belum ada pengeluaran di laporan ini.</div>
                    @endforelse
                </div>
            </div>
        </section>
                ...
                <div class="mt-4 space-y-3">
                    ...
                    @empty
                        <div class="py-8 text-center text-sm text-gray-500 dark:text-slate-400">
                            {{ $labelsReady ? 'Belum ada pengeluaran berlabel.' : 'Jalankan migrasi label untuk mengaktifkan laporan label.' }}
                        </div>
                    @endforelse
                </div>
            </div>

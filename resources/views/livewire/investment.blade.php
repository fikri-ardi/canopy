<div class="min-w-0" x-data="{deleteMovement: false}">
    <header class="app-header">
        <div class="page-header-layout">
            <div class="page-header-copy">
                ...
                <div class="min-w-0">
                    <div class="eyebrow">Investasi</div>
    ...
    <main class="space-y-6 px-4 py-5 sm:px-6 sm:py-6 lg:px-8">
        @unless ($schemaReady && $targetsReady)
            <section class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 shadow-sm dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
                Ledger investasi belum dimigrasi. Jalankan <span class="font-semibold">php artisan migrate</span> untuk mengaktifkan menu ini.
            </section>
        @endunless
        ...
        <section class="summary-grid">
            <div class="metric-card">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Modal Awal</div>
            ...
            <div class="metric-card">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Ditarik</div>
            ...
            <div class="metric-card">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Saldo Saat Ini</div>
            ...
            <div class="metric-card">
                <div class="flex items-center justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Progress Target</div>
                        <div class="mt-1 text-sm font-medium text-gray-500 dark:text-slate-400">Target keseluruhan</div>
                    </div>
                ...
                <div class="max-h-[34rem] divide-y divide-gray-100 overflow-y-auto dark:divide-slate-800">
                        ...
                        <button type="button" wire:click="selectInvestment(@js($group['key']))" wire:key="investment-item-{{ str($group['key'])->slug() }}" class="block w-full px-4 py-3 text-left transition hover:bg-gray-50 dark:hover:bg-slate-800/70 {{ ($selected['key'] ?? null) === $group['key'] ? 'bg-green-50/70 dark:bg-green-500/10' : 'bg-white dark:bg-slate-900' }}">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="shrink-0 text-right">
                                    ...
                                    <div class="mt-1 text-xs text-gray-400 dark:text-slate-500">saldo</div>
                                </div>
                            </div>
                            <div class="mt-3 flex items-center justify-between gap-3">
                                <div class="min-w-0 text-xs">
                                    <div class="text-gray-500 dark:text-slate-400">Target</div>
                                    <div class="money-value mt-0.5 truncate font-medium text-gray-500 dark:text-slate-400">{{ $group['target'] > 0 ? $this->rupiah($group['target']) : 'Belum diatur' }}</div>
                                </div>
                    ...
                    @empty
                        <div class="px-4 py-12 text-center text-sm">
                            ...
                            <div class="mt-3 font-semibold text-gray-950 dark:text-slate-50">Belum ada pengeluaran investasi</div>
                            <p class="mt-1 text-gray-500 dark:text-slate-400">Tambahkan pengeluaran dengan label Investasi lebih dulu.</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="min-w-0 space-y-4">
                <section class="panel overflow-hidden px-4 py-4">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div class="min-w-0">
                            <div class="text-xs font-semibold uppercase text-gray-400 dark:text-slate-500">Dipilih</div>
                            <h2 class="mt-1 truncate text-lg font-bold text-gray-950 dark:text-slate-50">{{ $selected['name'] ?? 'Belum ada investasi dipilih' }}</h2>
                        </div>
                        @if ($selected)
                            <div class="grid w-full min-w-0 grid-cols-2 gap-2 text-left text-xs sm:w-auto sm:grid-cols-4 sm:text-right">
                                <div>
                                    <div class="text-gray-400 dark:text-slate-500">Modal</div>
                                    <div class="money-value mt-1 truncate font-bold text-gray-950 dark:text-slate-50">{{ $this->rupiah($selected['principal']) }}</div>
                                </div>
                                <div>
                                    <div class="text-gray-400 dark:text-slate-500">Terpakai</div>
                                    <div class="money-value mt-1 truncate font-bold text-red-500">{{ $this->rupiah($selected['withdrawn']) }}</div>
                                </div>
                                <div>
                                    <div class="text-gray-400 dark:text-slate-500">Saldo</div>
                                ...
                                <div class="flex min-w-0 items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div class="mt-1 text-sm font-semibold text-gray-950 dark:text-slate-50">
                                            {{ $selected['targetProgress'] === null ? 'Atur target untuk melihat progress' : 'Memantau target terpilih' }}
                                        </div>
                                        @if ($selected['target'] > 0)
                                            <div class="money-value mt-1 text-xs text-gray-500 dark:text-slate-400">{{ $this->rupiah($selected['remainingToTarget']) }} menuju target</div>
                                        @endif
                                    </div>
                                </div>
                ...
                <section class="table-shell">
                    <table class="w-full min-w-[390px] table-fixed sm:min-w-[560px] lg:min-w-[620px]">
                        ...
                        <thead class="bg-gray-50 text-[10px] font-semibold uppercase text-gray-500 dark:bg-slate-950 dark:text-slate-400 sm:text-xs">
                            <tr>
                                <th class="px-2 py-2 text-left sm:p-3">Tanggal</th>
                                <th class="px-2 py-2 text-left sm:p-3">Tipe</th>
                                <th class="px-2 py-2 text-left sm:p-3">Catatan</th>
                                <th class="px-2 py-2 text-right sm:p-3">Nominal</th>
                                <th class="px-1.5 py-2 text-right sm:p-3">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 text-[13px] dark:divide-slate-800 sm:text-sm">
                                ...
                                <tr wire:key="investment-movement-{{ $movement->id }}" class="bg-white text-gray-800 transition hover:bg-gray-50 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800/60">
                                    ...
                                    <td class="px-2 py-2 sm:p-3">
                                        <span class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-semibold sm:px-2 sm:py-1 sm:text-xs {{ $movement->type === 'withdrawal' ? 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-300' : 'bg-green-50 text-green-600 dark:bg-green-500/10 dark:text-green-300' }}">
                                            <span class="sm:hidden">{{ $movement->type === 'withdrawal' ? 'Keluar' : 'Masuk' }}</span>
                                            <span class="hidden sm:inline">{{ $movement->type === 'withdrawal' ? 'Penarikan' : 'Top up' }}</span>
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-1.5 py-2 text-right sm:p-3">
                                        <button type="button" x-on:click="deleteMovement = true" wire:click="confirmDeleteMovement({{ $movement->id }})" class="btn-secondary size-8 px-0 py-0 text-xs sm:h-auto sm:w-auto sm:px-3 sm:py-1.5" aria-label="Hapus mutasi" data-tooltip="Hapus mutasi">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-3 py-10 text-center text-sm text-gray-500 dark:text-slate-400">Belum ada mutasi untuk investasi ini.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div x-show="deleteMovement" x-cloak x-transition class="modal-backdrop">
        <div x-on:click.away="deleteMovement = false" class="modal-panel">
            <div class="text-lg font-semibold text-gray-950 dark:text-slate-50">Hapus Mutasi</div>
            <p class="mt-3 text-sm text-gray-500 dark:text-slate-400">
                Hapus catatan {{ $deleteMovement?->type === 'withdrawal' ? 'penarikan' : 'top up' }} ini? Pengeluaran asal tetap tidak berubah.
            </p>

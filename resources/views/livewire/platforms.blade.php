<div class="min-w-0" x-data="{deletePlatform: false}">
    ...
    <main class="space-y-6 px-4 py-5 sm:px-6 sm:py-6 lg:px-8">
        @unless ($schemaReady)
            <section class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 shadow-sm dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
                Kepemilikan platform belum dimigrasi. Jalankan <span class="font-semibold">php artisan migrate</span> untuk mengaktifkan menu ini.
            </section>
        @endunless
        ...
        <section class="panel px-4 py-4">
            <form wire:submit="store" class="flex flex-col gap-2 sm:flex-row">
                <div class="relative min-w-0 flex-1">
                    ...
                    <input wire:model="name" type="text" placeholder="Tunai, Jago, BNI, ShopeePay" @disabled(! $schemaReady) class="input-field pl-9">
                </div>
                <button type="submit" @disabled(! $schemaReady || $errors->has('name')) wire:loading.attr="disabled" wire:target="store" class="btn-primary w-full sm:w-auto inline-flex items-center justify-center gap-2">
        ...
        <section class="table-shell">
            ...
            <div class="min-w-[620px] divide-y divide-gray-100 dark:divide-slate-800">
                @forelse ($platforms as $platform)
                    <div wire:key="platforms-row-{{ $platform->id }}" class="grid grid-cols-[1fr_120px_140px] items-center gap-3 bg-white px-4 py-3 text-sm transition hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800/60">
                        <div class="min-w-0">
                        ...
                        <div class="flex justify-end gap-2">
                            ...
                            @else
                                <button type="button" wire:click="startEditing({{ $platform->id }})" class="btn-secondary px-3 py-1.5 text-xs">Ubah</button>
                            @endif
                            <button type="button" x-on:click="deletePlatform = true" wire:click="confirmDelete({{ $platform->id }})" @disabled($platform->spends_count > 0) class="btn-danger px-3 py-1.5 text-xs disabled:cursor-not-allowed disabled:opacity-50">Hapus</button>
                        </div>
                    </div>
                @empty
                    <div class="px-4 py-12 text-center text-sm">
                        ...
                        <div class="mt-3 font-semibold text-gray-950 dark:text-slate-50">Belum ada platform</div>
                        <p class="mt-1 text-gray-500 dark:text-slate-400">Buat platform sebelum menambahkan pengeluaran.</p>
                    </div>
                @endforelse
            </div>
        </section>
    ...
    <div x-show="deletePlatform" x-cloak x-transition class="modal-backdrop">
        <div x-on:click.away="deletePlatform = false" class="modal-panel">
            <div class="text-lg font-semibold text-gray-950 dark:text-slate-50">Hapus Platform</div>
            <p class="mt-3 text-sm text-gray-500 dark:text-slate-400">
                Hapus <span class="font-semibold text-gray-950 dark:text-slate-50">{{ $platformToDelete?->name }}</span>? Hanya platform yang belum dipakai yang bisa dihapus.
            </p>
            ...
            <div class="mt-6 flex justify-end gap-2">
                <button type="button" x-on:click="deletePlatform = false" class="btn-secondary">Batal</button>
                <button type="button" x-on:click="deletePlatform = false" wire:click="delete" class="btn-danger">Hapus</button>
            </div>
        </div>
    </div>
</div>

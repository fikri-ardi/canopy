    ...
    <table class="w-full min-w-[700px] table-fixed sm:min-w-[840px]">
        <thead class="expense-table-head">
            <tr>
                @foreach ($sortColumns as $column => $config)
                    <th class="{{ $config['class'] }}">
                        <button
                            ...
                            aria-label="Urutkan berdasarkan {{ $config['label'] }}"
                        >
                ...
                @endforeach
                <th class="w-14 p-2 text-center sm:w-20 sm:p-3">Aksi</th>
            </tr>
        </thead>
            ...
            @empty
                <tr>
                    <td colspan="7" class="px-3 py-12 text-center">
                        ...
                        <div class="mt-3 font-semibold text-gray-950 dark:text-slate-50">Belum ada pengeluaran</div>
                        <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">Tambahkan pengeluaran pertama untuk melihatnya di sini.</p>
                    </td>
                </tr>
            @endforelse
        </tbody>
